...
$('#client_id').on("keypress",function(event) {
    //buscar el libro por codigo y agregarlo
    if ( event.which == 13)
    {
      agregaCliente($('#client_id').val());
    }
    else{
...
function agregaCliente(id){
  ...
  $.ajax({
    dataType: "json",
    url: "http://localhost:3000/clients/find.json?id=" + id})
  .done(function(data) {
      if(data.id == null){
     ...
     //  alert("Datos del libro: " + data.id + " codigo: " + data.email);
       // $('#nombre').click(function ()
        ...
        //  $('#client_id').val(data.id);
          ...
          $('#client_facebook').val(data.facebook);
          $('#client_linkedin').val(data.linkedin);
...
function agregaLibro(codigo){
  ...
  $.ajax({
    dataType: "json",
    url: "http://localhost:3000/products/find.json?codigo=" + codigo})
  .done(function(data) {
      if(data.id == null){
